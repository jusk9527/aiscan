...
单个二进制，零依赖，开箱即用。内置端口扫描、Web 探测、弱口令检测、POC 检测、爬虫五大引擎——不需要 LLM 也能完成完整的安全扫描流水线。接入 LLM 后解锁自然语言驱动的自主 agent：它自己选工具、自己跑扫描、自己分析证据、自己写报告。再加一台 IOA server，多个 agent 自动协作。

三种使用模式：**Scan**（确定性流水线，AI 可选辅助）、**Agent**（LLM 自主安全评估）、**IOA**（多 agent 分布式协作）。
